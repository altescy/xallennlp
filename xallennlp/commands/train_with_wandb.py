...
class TrainWithWandb(Subcommand):
    def add_subparser(
        self,
        parser: argparse._SubParsersAction,  # pylint: disable=protected-access
    ) -> argparse.ArgumentParser:
        description = """Train the model with Weights & Biases."""
        subparser = parser.add_parser(
            self.name,
            description=description,
            help="Train a model with Weights & Biases.",
        )
        ...
        subparser.add_argument(
            ...
            type=str,
            default="outputs",
            help="directory in which to save the artifacts before wandb logging",
        )
...
def train_model_from_args(args: argparse.Namespace) -> None:
    ...
    config = flatten_dict.flatten(params.as_dict(quiet=True), reducer="dot")
    run = wandb.init(dir=serialization_dir, config=config)
    if run is None:
        raise RuntimeError("Failed to init wandb.")

    with run:
        train_model(
            params=params,
            serialization_dir=os.path.join(serialization_dir, "allennlp"),
